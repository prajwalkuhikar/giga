...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    parser = argparse.ArgumentParser()
    parser.add_argument("cad_path", help="Path to the model file")
    parser.add_argument("obj_pose", help="Path to the object poses file")
    parser.add_argument("output_dir", help="Path to where the final files will be saved")
    parser.add_argument("gpus_devices", help="GPU devices")
    parser.add_argument("disable_output", help="Disable output of blender")
    parser.add_argument("scale_translation", help="Scale translation to meter")
    args = parser.parse_args()
    ...
    is_tless = "tless" in args.output_dir.lower()

    if args.disable_output.lower() == "true":
        logfile = os.path.join(args.output_dir, "render.log")
